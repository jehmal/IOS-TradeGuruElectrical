...
export function createOandPTool(tool) {
  return tool;
}
...
export async function generateObject(params) {
  getPostHogClient()?.capture("OANDP/GENERATE_OBJECT", {});
...
export async function generateText(params) {
  getPostHogClient()?.capture("OANDP/GENERATE_TEXT", {});
...
export function useOandPAgent(options) {
  const {
    addToolResult,
    ...rest
  } = useChat({
    ...
    onToolCall: async ({
      ...
      getPostHogClient()?.capture("OANDP/USE_AGENT/TOOL_CALL", {
  ...
  const sendMessage = useCallback(message => {
    getPostHogClient()?.capture("OANDP/USE_AGENT/SEND_MESSAGE", {});
